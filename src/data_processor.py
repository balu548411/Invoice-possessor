...
class InvoiceDataProcessor:
    ...
    def __init__(
        ...
        self.image_files = list(self.images_dir.glob("*.jpg"))[:10]
        if max_examples:
    ...
    def _process_json_labels(
        self, json_path: Path, image_shape: Tuple[int, int]
    ) -> Dict:
        """
        ...
        processed_data = {"fields": [], "boxes": [], "values": []}
    ...
    def extract_key_fields(self, processed_data: Dict) -> Dict:
        ...
        if "raw_data" in processed_data and isinstance(
            processed_data["raw_data"], dict
        ):
        ...
        for field, box, value in zip(
            processed_data["fields"], processed_data["boxes"], processed_data["values"]
        ):
            value_lower = value.lower()

            # Invoice number detection
            if ("invoice" in value_lower and "#" in value_lower) or (
                "invoice" in value_lower and "no" in value_lower
            ):
                words = value.split()
                for i, word in enumerate(words):
                    if i < len(words) - 1 and (
                        "invoice" in word.lower()
                        and ("#" in words[i + 1] or "no" in word.lower())
                    ):
            ...
            # Total amount
            if (
                "total" in value_lower
                or "amount" in value_lower
                or "balance" in value_lower
            ...
            if (
                ...
                or "seller" in value_lower
                or "from" in value_lower
            ):
            ...
            if (
                ...
                or "bill to" in value_lower
                or "buyer" in value_lower
            ):
            ...
            if (
                ...
                or "payment" in value_lower
                and "terms" in value_lower
            ):
            ...
            if "po" in value_lower and "number" in value_lower:
                ...
                for i, word in enumerate(words):
                    if i < len(words) - 1 and (
                        "po" in word.lower() and "#" in words[i + 1]
                    ):
    ...
    def create_dataset(self):
        ...
        for i, image_path in enumerate(self.image_files):
            try:
                print(
                    f"Processing image {i+1}/{len(self.image_files)}: {image_path.name}"
                )
